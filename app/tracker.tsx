...
import { localDate, mealSchema, shiftDate, type Meal } from "@/lib/meals";
...
export default function Tracker() {
  ...
  const [captureMode, setCaptureMode] = useState<"camera" | "photo" | "voice" | "text">("camera");
  ...
  return <main className="cal-shell">
    ...
    {addOpen && captureMode === "camera" && <CameraOverlay onClose={() => setAddOpen(false)} onPhoto={file => { setCapturedPhoto(file); setCaptureMode("photo"); }} onVoice={() => setCaptureMode("voice")} onText={() => setCaptureMode("text")}/>}
    <Dialog open={addOpen && captureMode !== "camera"} onOpenChange={setAddOpen}><DialogContent className="add-meal-dialog max-h-[90svh] overflow-y-auto"><DialogHeader className="sr-only"><DialogTitle>Add a meal</DialogTitle><DialogDescription>Describe your meal or add a photo or voice note to estimate its nutrition.</DialogDescription></DialogHeader><button className="back-to-camera" onClick={() => { setCapturedPhoto(null); setCaptureMode("camera"); }}><ArrowLeft size={16}/> Camera</button><Composer date={date} aiReady={settings?.aiReady ?? null} onReview={openMeal} resetKey={composerReset} initialPhoto={capturedPhoto} voiceFirst={captureMode === "voice"}/></DialogContent></Dialog>
    ...
    <Dialog open={draft !== null} onOpenChange={open => { if (!open && !busy) setDraft(null); }}><DialogContent className="max-h-[90svh] overflow-y-auto"><DialogHeader><DialogTitle>{draft?.createdAt ? "Edit meal" : "Review your estimate"}</DialogTitle><DialogDescription>Check the meal and portion before saving. Nutrition values are estimates.</DialogDescription></DialogHeader>
      {draft && <form className="meal-form" onSubmit={saveDraft}>
        <div className="review-detail"><span>Meal</span><strong>{draft.title}</strong></div>
        ...
        <Button type="submit" disabled={busy || !!correction.trim()} className="w-full">{busy ? <LoaderCircle className="animate-spin" /> : <Plus />}{busy ? "Saving…" : "Save meal"}</Button>
      </form>}
