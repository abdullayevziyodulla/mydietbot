import { z } from "zod";
import { apiKey, MODELS, openai, reserveCall } from "@/lib/ai";
...
export async function POST(request: Request) {
  try {
    ...
    const data = z.object({ text: z.string().trim().min(1).max(4000) }).safeParse(await openai("audio/transcriptions", body, key));
